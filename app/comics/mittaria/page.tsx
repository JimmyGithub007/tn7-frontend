...
const content = [
    { id: "1", title: "PRELUDE", date: "15 APRIL 2025" },
...
const Mittaria = () => {
    ...
    return (<div className="fixed h-screen w-full overflow-hidden flex justify-center">
        <Image id="background" className="absolute top-0 left-0 w-full h-full object-cover" alt="" width={5760} height={3260} src={`/assets/images/comics/${id}/webp/Background.webp`} priority />
        <Loader />
                ...
                >
                    <div className={`flex flex-col gap-8 md:justify-center px-2 text-white z-10`}>
                        <div className={`flex flex-col gap-2`}>
                            <div className="text-lg sm:text-xl md:text-2xl xl:text-3xl">Mittaria Webtoon</div>
                            <div className={`text-xs sm:text-sm md:text-md lg:text-lg italic ${opinionPro.className}`}>
                                After completing her trials and adventure in the mortal realm, Mitria regained her immortal powers and found that she was stronger than ever.
                                Using her newfound abilities, she created a galaxy to reflect her loving and luminous personality. This unique star system would become known as Mittaria.<br /><br />
                                ...
                                content.map((value, key) => (
                                    <Link href={`/comics/${id}/${value.id}`} key={key} className="cursor-pointer duration-300 flex group items-center justify-between hover:bg-slate-50/10 text-xs sm:text-sm pr-2">
                                        <div className={`flex items-center gap-8 ${opinionPro.className}`}>
                                            <Image className="duration-300 group-hover:saturate-150 w-12 sm:w-16" alt="prelude" height={315} width={315} src={`/assets/images/comics/mittaria/prelude/prelude.png`} priority />
                                            {value.title}
                                        </div>
                                        <div>{value.date}</div>
                                    </Link>
